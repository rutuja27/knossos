...
public:
    ...
    using MeshResult = std::tuple<boost::optional<decltype(treeListElement::treeID)>, QVector<float>, QVector<float>, QVector<unsigned int>, QVector<std::uint8_t>>;
    MeshResult loadMesh(QIODevice &, const boost::optional<decltype(treeListElement::treeID)> treeID, const QString & filename);
